...
export const createUserController = async (req, res, next) => {
    try {
        ...
        if (!userName) {
            next(new CustomError(CustomConstants.joiError, "Please enter an username."));
        }
...
export const deleteUserController = async (req, res, next) => {
    try {
        ...
        const existingUser = await UserModel.findOne({ userName });
        if (!existingUser) {
            next(new CustomError(CustomConstants.notFoundError, "User with this username doesn't exists."));
        }
